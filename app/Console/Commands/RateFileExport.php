...
class RateFileExport extends Command {
	...
	public function fire()
	{
		...
		Log::useFiles(storage_path() . '/logs/ratefileexport-' . $CompanyGatewayID . '-' . date('Y-m-d') . '.log');
		$joblogdata = array();
		$joblogdata['Message'] = '';
		...
		try {
			...
			if (isset($cronsetting["ScriptLocation"]) && !empty($cronsetting["ScriptLocation"])) {
				$command = "php " . $cronsetting["ScriptLocation"] . "/artisan streamcoratefileimporter ";
