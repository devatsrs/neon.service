...
class HUAWEIDownloadCDR extends Command {
     ...
     * @var string
     */
    protected $name = 'huaweidownloadcdr';
    ...
    public function handle()
    {
        ...
        Log::useFiles(storage_path().'/logs/huaweidownloadcdr-'.$CompanyGatewayID.'-'.date('Y-m-d').'.log');
        ...
        try {
            ...
            $huawei = new HUAWEI($CompanyGatewayID);
            Log::info("HUAWEI Connected");
            $filenames = $huawei->getCDRs();
            $destination = $HUAWEI_LOCATION .'/'.$CompanyGatewayID;
            if (!file_exists($destination)) {
                mkdir($destination, 0777, true);
            }
            //$filenames = UsageDownloadFiles::remove_downloaded_files($CompanyGatewayID,$filenames);
            Log::info('huawei File download Count '.count($filenames));
            $downloaded = array();
            if(count($filenames) > 0) {
                ...
                foreach ($filenames as $filename) {
                    ...
                    if (!file_exists($file_path)) {
                        ...
                        Log::info("HUAWEI downloaded file" . $filename);
                        $downloaded[] = $filename;
                        //$huawei->deleteCDR($param);
                        $isdownloaded = true;

                    }else {
                        //Log::info("HUAWEI File was already exist  " . $filename . ' - ' . $huawei->get_file_datetime($filename));
                    }
                    ...
                    if(filesize($file_path) > 0 &&  UsageDownloadFiles::where(array("CompanyGatewayID" => $CompanyGatewayID, "FileName" => basename($filename)))->count() == 0 ) {
                        if($isdownloaded == false){
                            ...
                            $param['download_path'] = $destination . '/';
                            $huawei->downloadCDR($param);
                            Log::info("Missing file inserted " . $filename . ' - ' . $huawei->get_file_datetime($filename));
                        }
            ...
            if (count($downloaded) > 0) {
                ...
                $joblogdata['Message'] .= "<br> Date  : " . $huawei->get_file_datetime($downloaded[0]);
                $joblogdata['Message'] .= " - " . $huawei->get_file_datetime($downloaded[$downloaded_files - 1]);
            }
            ...
            Log::info("HUAWEI file Download Completed ");
