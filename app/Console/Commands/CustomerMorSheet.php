...
class CustomerMorSheet extends Command {
     ...
     * @var string
     */
    protected $name = 'customermorsheet';
    ...
    public function handle()
    {
        ...
        try{
            ...
            //$excel_data = DB::select("CALL  prc_CronJobGenerateMorSheet( '" .$job->AccountID . "','" . $tunkids."','".$Effective."' ) ");
            $query = "CALL  prc_CronJobGenerateMorSheet( '" .$job->AccountID . "','" . $tunkids."'," . $timezoneid.",'".$Effective."','".$CustomDate."' ) ";
            //Log::info($query);
            ...
            $jobdata['JobStatusMessage'] = 'Mor Customer Rate File Generated Successfully';
            ...
            $jobdata['JobStatusMessage'] = 'Mor Customer Rate Download Failed::'.$e->getMessage();
            $jobdata['updated_at'] = date('Y-m-d H:i:s');
            $jobdata['ModifiedBy'] = 'RMScheduler';
            Job::where(["JobID" => $JobID])->update($jobdata);
            Log::error($e);
