...
class VendorMorSheet extends Command {
    ...
    public function handle()
    {
        ...
        Log::useFiles(storage_path().'/logs/morvendorsheet-'.$JobID.'-'.date('Y-m-d').'.log');
        ...
        try{
            ...
            $query = "CALL  prc_CronJobGenerateMorVendorSheet ('" .$job->AccountID . "','" . $tunkids."'," . $timezoneid.",'".$Effective."','".$CustomDate."',".$AppliedToVendor.",".$VoiceCallTypeID.")";
            Log::info($query);
            ...
            $jobdata['JobStatusMessage'] = 'Mor Vendor File Generated Successfully';
            ...
            $jobdata['JobStatusMessage'] = 'Mor Vendor File Download Failed::'.$e->getMessage();
            $jobdata['updated_at'] = date('Y-m-d H:i:s');
            $jobdata['ModifiedBy'] = 'RMScheduler';
            Job::where(["JobID" => $JobID])->update($jobdata);
            Log::error($e);
