...
class PortaOneAccountUsage extends Command {
     ...
     * @var string
     */
    protected $name = 'portaoneaccountusage';
    ...
    public function fire()
    {
        ...
        Log::useFiles(storage_path() . '/logs/portaoneaccountusage-' . $CompanyGatewayID . '-' . date('Y-m-d') . '.log');
        ...
        try {
            ...
            if(isset($cronsetting['CDRImportStartDate']) && trim($cronsetting['CDRImportStartDate'])!=''){
                ...
                $porta = new PortaOne($CompanyGatewayID);
                $responselistAccounts = $porta->listAccounts();
                //Log::info("customers  - ".print_R($responselistAccounts,true));
                if (isset($responselistAccounts['customer_list'])) {
                    foreach ((array)$responselistAccounts['customer_list'] as $row_account) {

                        $gadata = array();
                        $gadata['CompanyID'] = $CompanyID;
                        $gadata['CompanyGatewayID'] = $CompanyGatewayID;
                        $gadata['ServiceID'] = $ServiceID;
                        $gadata['GatewayAccountID'] = $row_account['i_customer'];
                        $gadata['AccountIP'] = '';
                        $gadata['AccountNumber'] = '';
                        $gadata['AccountCLI'] = '';
                        $accounts[$row_account['i_customer']] = $row_account['name'];

                        $gadata['AccountName'] = $row_account['name'];
                        $row_account['CreationDate'] = date("Y-m-d H:i:s", (doubleval(filter_var($row_account['creation_date'], FILTER_SANITIZE_NUMBER_INT)) / 1000));
                        $gadata['AccountDetailInfo'] = json_encode($row_account);

                        //Log::info("customers  - ".print_R($gadata,true));exit;
                        if (GatewayAccount::where(array('CompanyGatewayID' => $CompanyGatewayID, 'CompanyID' => $CompanyID, 'ServiceID' => $ServiceID, 'GatewayAccountID' => $row_account['i_customer']))->count()) {
                            GatewayAccount::where(array('CompanyGatewayID' => $CompanyGatewayID, 'CompanyID' => $CompanyID, 'ServiceID' => $ServiceID, 'GatewayAccountID' => $row_account['i_customer']))->update($gadata);
                        } else {
...
//exit;
                    if (!isset($response['faultCode'])) {
                        if (isset($response['xdr_list'])) {
                            Log::error('call count ' . count($response['xdr_list']) . ' GatewayAccountID = ' . $GatewayAccountID);
                            foreach ((array)$response['xdr_list'] as $row_account) {
                                $data = array();
                                $data['CompanyGatewayID'] = $CompanyGatewayID;
                                $data['CompanyID'] = $CompanyID;
                                $data['GatewayAccountID'] = $row_account['i_customer'];
                                ...
                                $data['duration'] = $row_account['unix_disconnect_time'] - $row_account['unix_connect_time'];
                                ...
                                $data['ID'] = $row_account['i_xdr'];
                                if(isset($row_account['destination']) && $row_account['destination']  == 'INCOMING') {
                                    $data['is_inbound'] = 1;
                                }
                                else{
                                    $data['is_inbound'] = 0;
                                ...
                                if ((isset($row_account['destination']) && $row_account['destination'] == 'INCOMING')  ||
                                    (isset($row_account['destination']) && is_numeric($row_account['destination']) ))
                                {
                                    $InserData[] = $data;
                                    $data_count++;
                                }
                ...
                //Log::error("Porta CALL  prc_insertCDR ('" . $processID . "', '" . $temptableName . "' ) start");exit;
